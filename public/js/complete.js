...
async function checkPaymentStatus() {
  ...
  if (!piClientSecret) {
    console.debug("No payment intent client secret found");
    return;
  } else {
    console.log("piClientSecret", piClientSecret);
  }
...
function setPaymentDetails(intent) {
    ...
    switch (intent.status) {
      ...
      const dollars = intent.amount / 100;
      document.querySelector("#intent-amount").textContent = dollars.toFixed(2);
      // document.querySelector("#intent-amount").dataset.amount = intent.amount;

      document.querySelector("#intent-status").textContent = intent.status;
      document.querySelector("#receipt-email").textContent = intent.receipt_email
